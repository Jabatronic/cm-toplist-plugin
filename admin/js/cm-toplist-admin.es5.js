(function () {
  var vm = new Vue({
    ...
    methods: {
      delete_item: function (index, delete_id, delete_name) {
        ...
        if (confirm(`Are you sure you wish to remove ${delete_name} from the database? `)) {
          axios.delete(`${wpApiSettings.url}?_wpnonce=${wpApiSettings.nonce}&brand_id=${delete_id}`).then(response => {
            console.log(response);
            this.cm_toplist_data.splice(index, 1); // Output response msg

            this.msg.push(response.data);
          }).catch(e => {
            console.log(e); // add the new errors
      ...
      add_new: function () {
        // empty previous errors
        this.msg = [];
        axios.post(`${wpApiSettings.url}?_wpnonce=${wpApiSettings.nonce}`, {
          ...
          this.msg.push(response.data); // empty the input data
    ...
    mounted: function () {
      ...
      axios.get(wpApiSettings.url).then(response => {
